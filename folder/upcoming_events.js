...
.then((datos)=>{
    ...
    console.log(datos);
    let arrayEvents=datos.events
    console.log(arrayEvents);
    //Llamando las funciones

    mostrarTarjetas(arrayEvents)
    ...
    function filtro(){
        let primerfiltro = filtrarPorTexto(arrayEvents,input.value)
    ...
    function mostrarTarjetas(array){ 
        ...
        let tarjetas =""
        array.map(elemento=>{
            if(elemento.date>=datos.currentDate){
                tarjetas+=`<div class="card" style="width: 30rem; height:40rem;">
                ...
                <a href="details.html?id=${elemento._id}" class="btn btn-primary">Ver más</a>
                </div>
                </div>`
            }
        })
